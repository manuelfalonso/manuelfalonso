...
{
    /// <summary>
    /// Strategy for asynchronously or synchronously loading a scene by its scene path, with optional parameters. 
    /// If a single mode scene is loaded, Unity calls Resources.UnloadUnusedAssets automatically.
    /// </summary>
    [CreateAssetMenu(fileName = "NewLoadSceneByPathStrategy", menuName = "Sombra Studios/Strategies/Scenes/Load Scene by Path")]
    public class LoadSceneByPathStrategy : SceneStrategy<string>
    {
        ...
        {
            if (AsyncLoad)
                CoroutineManager.Instance.StartCoroutine(LoadSceneAsync(scenePath, LoadSceneParameters));
            else
                SceneManager.LoadScene(scenePath, LoadSceneParameters);
        }
